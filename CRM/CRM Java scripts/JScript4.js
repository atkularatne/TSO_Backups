...
function addBankLink() {
    ...
    var fetchXmlFindRelatedBank = "<fetch distinct='false' mapping='logical' output-format='xml-platform' version='1.0' >" +
                            "<entity name='wl_bank' >" +
                                ...
                                "<attribute name='inpswl_pensionnumber' />" +
                                "<order descending='false' attribute='wl_name' />" +
                                "<link-entity name='account' alias='ae' to='inpswl_pensionid' from='accountid' >" +
                                    "<filter type='and' >" +
...
"<entity name='inpswl_paymentmetadata' >" +
"<attribute name='inpswl_paymentmetadataid' />" +
"<attribute name='inpswl_name' />" +
"<order descending='false' attribute='inpswl_name' />" +
"<filter type='and' >" +
"<condition attribute='inpswl_pensionnumber' value='" + pensionNumber + "' operator='eq' />" +
"</filter>" +
"</entity>" +
"</fetch>";
...
function openPaymentRecord() {
    ...
    if (relatedPayments.length >= 1) {
        paymentGuid = relatedPayments[0].attributes.inpswl_paymentmetadataid.value;
        OpenRecords(paymentGuid, "inpswl_paymentmetadata", "");
    }
}
