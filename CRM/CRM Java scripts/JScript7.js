...
function openPayment() {
    ...
    switch (paymentType) {
        case "Cash":
            entity_logical_name = "wl_cashpayment"; //
            entity_attribute_name = "wl_cashpaymentid";
            break;
        case "Wire Transfer": //online tranfer
            entity_logical_name = "wl_onlinetransferpayment";
            entity_attribute_name = "wl_onlinetransferpaymentid";
            break;
        case "Remote Cheque":
            entity_logical_name = "wl_chequepayment"; //
            entity_attribute_name = "wl_chequepaymentid";
            break;
        case "SEPA":
            entity_logical_name = "wl_sepapayment"; //
            entity_attribute_name = "wl_sepapaymentid";
            break;
        case "ACH":
            entity_logical_name = "wl_achpayment";
            entity_attribute_name = "wl_achpaymentid"; //
            break;
        case "Masspay":
            //entity_logical_name = "wl_cashpayment";
            //entity_attribute_name = "wl_cashpaymentid";
    ...
    var fetchXmlFindPayments = "<fetch mapping='logical' version='1.0'>" +
            ...
            "<filter>" +
            "<condition attribute='wl_paymentid' operator='eq' value='" + paymentId + "' />" +
            "</filter>" +
            "</entity>" +
            "</fetch>";
    ...
    if (payments != null) {
        ...
        switch (paymentType) {
            case "Cash":
                recordGuid = payments[0].attributes.wl_cashpaymentid.value; //
                break;
            case "Wire Transfer": //online transfer
                recordGuid = payments[0].attributes.wl_onlinetransferpaymentid.value;
                break;
            case "Remote Cheque":
                recordGuid = payments[0].attributes.wl_chequepaymentid.value; //
                break;
            case "SEPA":
                recordGuid = payments[0].attributes.wl_sepapaymentid.value; //
                break;
            case "ACH":
                recordGuid = payments[0].attributes.wl_achpaymentid.value; //
                break;
            case "Masspay":
                //recordGuid = payments[0].attributes.wl_cashpaymentid.value;
                break;
        ...
        //OpenRecords(payments[0].attributes.wl_cashpaymentid.value, "wl_cashpayment", "");
